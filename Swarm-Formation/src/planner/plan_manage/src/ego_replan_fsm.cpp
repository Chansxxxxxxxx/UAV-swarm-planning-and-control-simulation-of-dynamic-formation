...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/playmode", playmode_, -1);
    ...
    for (int i = 0; i < 7; i++)
    {
      nh.param("global_goal/relative_pos_Y" + to_string(i) + "/x", swarm_relative_pts_Y[i][0], -1.0);
      nh.param("global_goal/relative_pos_Y" + to_string(i) + "/y", swarm_relative_pts_Y[i][1], -1.0);
      nh.param("global_goal/relative_pos_Y" + to_string(i) + "/z", swarm_relative_pts_Y[i][2], -1.0);
    }

    for (int i = 0; i < 7; i++)
    {
      nh.param("global_goal/relative_pos_S" + to_string(i) + "/x", swarm_relative_pts_S[i][0], -1.0);
      nh.param("global_goal/relative_pos_S" + to_string(i) + "/y", swarm_relative_pts_S[i][1], -1.0);
      nh.param("global_goal/relative_pos_S" + to_string(i) + "/z", swarm_relative_pts_S[i][2], -1.0);
    }

    for (int i = 0; i < 7; i++)
    {
      nh.param("global_goal/relative_pos_U" + to_string(i) + "/x", swarm_relative_pts_U[i][0], -1.0);
      nh.param("global_goal/relative_pos_U" + to_string(i) + "/y", swarm_relative_pts_U[i][1], -1.0);
      nh.param("global_goal/relative_pos_U" + to_string(i) + "/z", swarm_relative_pts_U[i][2], -1.0);
    }
    ...
    {
      if (playmode_ == 1)//值为1代表点一次目标点，即可自动变队形SYSU；值为2代表需要分别点3个目标点使其按次序变换队形SYSU
        central_goal = nh.subscribe("/move_base_simple/goal", 1, &EGOReplanFSM::myformation, this);
      if (playmode_ == 2)
  ...
  void EGOReplanFSM::myformation(const geometry_msgs::PoseStampedPtr &msg){ 
     goal_U = msg;                    //设置中间节点goal_Y,goal_S,最终目标点为goal_U
    
     goal_Y.pose.position.x = -7.0;
     goal_Y.pose.position.y = 0.0;
     goal_Y.pose.position.z = 0.0;

     goal_S.pose.position.x = 5.0;
     goal_S.pose.position.y = 0.0;
     goal_S.pose.position.z = 0.0;

     // 设置定时器回调函数             
    timer_ = nh_.createTimer(ros::Duration(0.001), &EGOReplanFSM::timerCallback, this);
    // 启动定时器  逻辑为分别以goal_Y,goal_S,goal_U为目标点运行formationWaypointCallback函数
    stage_ = 0;
    timer_.start();
  }

  void EGOReplanFSM::timerCallback(const ros::TimerEvent& event) {
    switch (stage_) {   
        case 0:
            // 运行一次 formationWaypointCallback
            formationWaypointCallback(boost::make_shared<geometry_msgs::PoseStamped>(goal_Y));
            // 切换到下一个阶段，设置定时器延时为 16s
            stage_ = 1;
            timer_.setPeriod(ros::Duration(16.0));
            break;
        case 1:
            // 运行一次 formationWaypointCallback
            formationWaypointCallback(boost::make_shared<geometry_msgs::PoseStamped>(goal_S));
            // 切换到下一个阶段，设置定时器延时为 14s
            stage_ = 2;
            timer_.setPeriod(ros::Duration(14.0));
            break;
        case 2:
            // 运行一次 formationWaypointCallback
            formationWaypointCallback(goal_U);
            // 完成所有阶段，停止定时器
            stage_ = 3;
            timer_.stop();
            break;
  ...
  void EGOReplanFSM::formationWaypointCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    ...
    relative_pos << swarm_relative_pts_Y[id][0],
                    swarm_relative_pts_Y[id][1],
                    swarm_relative_pts_Y[id][2];
    
    //根据num_formation的值来确定运行Y、S、U哪个姿态，每次运行formationWaypointCallback后num_formation会加1
    if (num_formation == 2){
      relative_pos << swarm_relative_pts_S[id][0],
                      swarm_relative_pts_S[id][1],
                      swarm_relative_pts_S[id][2];
      visualization_->formation_type_= visualization_->formation_type_+1;
      visualization_->initSwarmGraphVisual(); //根据formation_type_的值来决定连线形状
      
    }

    if (num_formation == 3){
      relative_pos << swarm_relative_pts_U[id][0],
                      swarm_relative_pts_U[id][1],
                      swarm_relative_pts_U[id][2];
      visualization_->formation_type_= visualization_->formation_type_-1;
      visualization_->initSwarmGraphVisual();
      
    }

    num_formation = (num_formation % 3) + 1;  // 在1和3之间循环
  ...
  void EGOReplanFSM::planGlobalTrajbyGivenWps()
  {
    ...
    num_formation = (num_formation % 2) + 1;  // 在1和2之间循环
