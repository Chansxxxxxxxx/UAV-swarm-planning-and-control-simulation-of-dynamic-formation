...
namespace ego_planner
{
  ...
  private:
    ...
    int num_formation;  //记录第几次运行formationwaypoint函数
    ros::Timer timer_;
    ros::NodeHandle nh_;  // 添加类成员变量
    // 计数器和状态
    int stage_ = 0;
    // 目标点
    geometry_msgs::PoseStamped goal_Y;
    geometry_msgs::PoseStamped goal_S;
    geometry_msgs::PoseStampedPtr goal_U;
    int playmode_;  //值为1代表点一次目标点，即可自动变队形SYSU；值为2代表需要分别点3个目标点使其按次序变换队形SYSU
    ...
    double swarm_relative_pts_S[50][3];
    double swarm_relative_pts_Y[50][3];
    double swarm_relative_pts_U[50][3];
    double swarm_scale_;
    ...
    void myformation(const geometry_msgs::PoseStampedPtr &msg);
